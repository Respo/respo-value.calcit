...
{} (:package |respo-value)
  :configs $ {} (:init-fn |respo-value.main/main!) (:reload-fn |respo-value.main/reload!) (:modules $ [] |lilac/ |memof/ |respo.calcit/) (:version |0.4.0)
  :files $ {}
    ...
    |respo-value.main $ {}
      :ns $ quote
        ns respo-value.main $ :require ([] hsl.core :refer $ [] hsl) ([] respo.core :refer $ [] render! clear-cache! realize-ssr!) ([] respo.cursor :refer $ [] update-states) ([] respo-value.comp.container :refer $ [] comp-container) ([] respo-value.schema :as schema) ([] respo.util.id :refer $ [] get-id!)
      :defs $ {}
        ...
        |updater $ quote
          defn updater (store op-type op-data op-id) (; println store op-type op-data)
            case op-type (:states $ update-states store op-data) (op-type store)
      :proc $ quote ()
    |respo-value.style.widget $ {}
      :ns $ quote
        ns respo-value.style.widget $ :require ([] respo.util.format :refer $ [] hsl)
      :defs $ {}
        |literal $ quote
          def literal $ {} (:line-height |16px) (:border-radius |4px) (:padding "|0px 4px") (:color $ hsl 0 0 30) (:font-family "|Source Code Pro, menlo, monospace") (:font-size |12px) (:display |inline-block) (:margin |4px)
            :box-shadow $ str "|0 0 1px " (hsl 0 0 0 0.3)
            :vertical-align |top
        |only-text $ quote
          def only-text $ {} (:pointer-events |none)
        |structure $ quote
          def structure $ {} (:line-height |16px) (:border-radius |4px) (:padding "|0px 2px") (:color $ hsl 0 0 40) (:font-family "|Source Code Pro, menlo, monospace") (:font-size |12px) (:display |inline-block) (:margin |4px)
            :box-shadow $ str "|0 0 1px " (hsl 0 0 0 0.3)
            :vertical-align |top
            :cursor |pointer
        ...
        |style-unknown $ quote (def style-unknown $ {})
      :proc $ quote ()
    |respo-value.schema $ {}
      :ns $ quote
        ns respo-value.schema $ :require ([] clojure.string :as string)
      :defs $ {}
        |a-hash-map $ quote (def a-hash-map $ {,} :a 1 :b 2)
        |a-list $ quote (def a-list $ [] 1 2 3 4)
        |a-function $ quote
          def a-function $ fn (x) (+ x 1)
        |a-bool $ quote (def a-bool true)
        |a-hash-set $ quote (def a-hash-set $ #{} 1 2 3)
        |a-vector $ quote (def a-vector $ [] 1 2 3)
    ...
    |respo-value.comp.container $ {}
      :ns $ quote
        ns respo-value.comp.container $ :require ([] respo.util.format :refer $ [] hsl) ([] respo-value.comp.value :refer $ [] comp-value) ([] respo-value.schema :as schema) ([] respo.comp.space :refer $ [] =<) ([] respo.comp.inspect :refer $ [] comp-inspect) ([] respo.core :refer $ [] defcomp <> div span list-> >>) ([] respo-value.style.layout :as layout) ([] respo-value.style.widget :as widget)
      :defs $ {}
        |comp-container $ quote
          defcomp comp-container (store)
            let
                states $ :states store
              div ({} $ :style layout/container) (span $ {})
                list-> ({})
        ...
        |style-value $ quote (def style-value $ {})
      :proc $ quote ()
    |respo-value.style.decoration $ {}
      :ns $ quote
        ns respo-value.style.decoration $ :require ([] respo.util.format :refer $ [] hsl)
      :defs $ {}
        |folded $ quote
    ...
    |respo-value.comp.value $ {}
      :ns $ quote
        ns respo-value.comp.value $ :require ([] hsl.core :refer $ [] hsl) ([] respo-value.style.widget :as widget) ([] respo-value.style.layout :as layout) ([] respo-value.style.decoration :as decoration) ([] respo.core :refer $ [] defcomp <> div span list-> >>)
      :defs $ {}
        |comp-map $ quote
            ...
            let
              ...
              cond
                ...
                (or (= x true) (= x false))
                  comp-bool x
                (set? x)
                  comp-set states x level
                (list? x)
                  comp-list states x level
                (map? x)
                  comp-map states x level
                true $ div
